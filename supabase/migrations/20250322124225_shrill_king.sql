/*
  # Add Response Time Tracking and Update Lead Scoring

  1. Changes
    - Add response_time column to leads table
    - Update lead scoring function to include AI readiness and response time
    - Add trigger to automatically calculate response time
    - Update existing lead scores with new weights

  2. Scoring Updates
    - AI Readiness: 25 points max
    - Response Time: Score * 1.3 multiplier
*/
...
BEGIN
  ...
  IF ai_readiness IS NOT NULL THEN
    CASE ai_readiness
      WHEN 'AI Competent' THEN ai_score := 25
      WHEN 'AI Ready' THEN ai_score := 20
      WHEN 'AI Aware' THEN ai_score := 15
      WHEN 'AI Unaware' THEN ai_score := 5
      ELSE ai_score := 0
    END;
  END IF;
...
BEGIN
  ...
  IF NEW.last_contacted IS NOT NULL AND OLD.last_contacted IS NULL THEN
